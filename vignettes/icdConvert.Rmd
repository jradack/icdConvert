...
We can use the `map_code` function, which takes a vector of ICD codes and converts them to the desired ICD version.
...
In addition to converting an exact ICD code to its counterpart, the function `map_code` allows for a user to search for all ICD codes matching a prefix.
This is useful when converting from ICD 9 to ICD 10, where ICD 9 used identified subcodes using an overarching code as the prefix.
...
```{r echo = FALSE, results='asis'}
knitr::kable(map_code_prefix[1:10,])
```
